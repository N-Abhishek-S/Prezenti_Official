...
async function sendWhatsApp(config: NotificationConfig, payload: ExpertInquiryFormValues, timestamp: string) {
  ...
  if (config.twilioContentSid) {
    try {
      ...
      console.error('Twilio Content API notification failed; falling back to direct WhatsApp body.', error);
    }
  }
...
export default async function handler(request: ApiRequest, response: ApiResponse) {
  ...
  try {
    const requestBody = readBody(request.body);
    const normalized = normalizeBody(requestBody);
    const validation = validateInquiryForm(normalized, allowedServices);

    if (!validation.isValid) {
      return response.status(400).json({ success: false, message: 'Please complete the required details.' });
    }
    ...
    if (failed) {
      ...
      return response.status(502).json({ success: false, message: 'Unable to send inquiry. Please try again.' });
    }

    return response.status(200).json({ success: true, message: successMessage });
  } catch (error) {
    console.error('Inquiry notification error', error);
    return response.status(500).json({ success: false, message: 'Unable to send inquiry. Please try again.' });
  }
}
