...
// api/inquiry-notification.ts
var inquiry_notification_exports = {};
__export(inquiry_notification_exports, {
  default: () => handler
});
module.exports = __toCommonJS(inquiry_notification_exports);
...
var successMessage = "Inquiry sent successfully. Our team has been notified.";
var requestTimeoutMs = 18e3;
var maxJsonBodyBytes = 32e3;
...
function getTodayDateValue() {
  ...
  const localDate = new Date(today.getTime() - offset * 6e4);
...
function sanitizeInquiryForm(values) {
  return {
...
function validateInquiryForm(values, availableServices = []) {
...
function formatTimestamp(submittedAt) {
  const date = submittedAt ? new Date(submittedAt) : /* @__PURE__ */ new Date();
...
function getSelectedServices(payload) {
  return payload.services.length > 0 ? payload.services.join(", ") : "No services selected.";
}
function formatDisplayDate(value) {
  const date = /* @__PURE__ */ new Date(`${value}T00:00:00+05:30`);
...
function buildEmailBody(payload, timestamp) {
...
function buildEmailHtmlBody(payload, timestamp) {
  const selectedServices = getSelectedServices(payload);
  const additionalRequirement = getDisplayValue(payload.additionalRequirement, "No additional requirement provided.");
...
function buildWhatsAppBody(payload, timestamp) {
  const selectedServices = getSelectedServices(payload);
  const additionalRequirement = getDisplayValue(payload.additionalRequirement, "No additional requirement provided.");
  return [
    "*New Business Inquiry | Prezenti*",
    ...
    "",
    "\u{1F6E0} *Service Requirement*",
    `Required Start Date: ${formatDisplayDate(payload.requiredStartDate)}`,
    `Selected Services: ${selectedServices}`,
    "",
    "\u{1F4DD} *Additional Requirement*",
    additionalRequirement,
    "",
    "\u{1F4CC} Source: Prezenti Website",
    `\u{1F552} Submitted: ${timestamp}`,
    "",
    "Please follow up with the customer promptly."
  ].join("\n");
}
function buildWhatsAppContentVariables(payload, timestamp) {
...
async function sendEmail(config, payload, timestamp) {
  try {
    ...
    console.log("[SMTP SUCCESS] Email sent to", config.emailTo);
  } catch (err) {
...
async function sendWhatsApp(config, payload, timestamp) {
  try {
    ...
    if (config.twilioContentSid) {
      try {
        ...
        console.log("[TWILIO SUCCESS] WhatsApp template sent to", config.twilioWhatsAppTo);
        return;
      } catch (error) {
    ...
    console.log("[TWILIO SUCCESS] WhatsApp message sent to", config.twilioWhatsAppTo);
  } catch (err) {
    console.error("[TWILIO ERROR] Failed to send WhatsApp:", err.message);
    throw { stage: "TWILIO", error: err.message || "Twilio API failed" };
  }
}
async function sendTwilioMessage(config, message) {
...
async function handler(request, response) {
  ...
  try {
    ...
    console.log("[CONFIG CHECK] Missing Variables:", missingVars.length ? missingVars.join(", ") : "None");
    assertConfig(config);
  } catch (error) {
  ...
  try {
    ...
    const results = await Promise.allSettled([
      sendEmail(config, validation.sanitized, timestamp),
      sendWhatsApp(config, validation.sanitized, timestamp)
    ]);
    const failures = results.filter((r) => r.status === "rejected");
    if (failures.length > 0) {
      const errors = failures.map((f) => f.reason);
      console.error("[DELIVERY FAILURE] One or more delivery methods failed:", JSON.stringify(errors));
      return response.status(502).json({ success: false, message: "Delivery failed", errors });
    }
